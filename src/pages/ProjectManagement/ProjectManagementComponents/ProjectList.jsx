                    ...
                    <td className="p-4">
                      <div className="flex -space-x-2">
                        {Array.isArray(project.team) && project.team.map((member, i) => (
                          <div key={i} title={member.name} className={`w-8 h-8 rounded-full border-2 ${isDark ? "border-[#111827]" : "border-white"} flex items-center justify-center text-[10px] font-bold ${member.bg}`}>
                            {member.initial}
                          </div>
                        ))}
                      </div>
