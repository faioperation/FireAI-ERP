...
//       if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
//         setIsOpen(false);
//         setSearchTerm(""); // ড্রপডাউন বন্ধ হলে সার্চ ক্লিয়ার হবে
//       }
//     };
//     document.addEventListener("mousedown", handleClickOutside);
//     return () => document.removeEventListener("mousedown", handleClickOutside);
...
//       opt.toLowerCase().includes(searchTerm.toLowerCase())
//     );
//     const updated = [...new Set([...selectedItems, ...filtered])];
//     setSelectedItems(updated);
//     onFilterChange(columnKey, updated);
//   };
...
//   // শুধুমাত্র সার্চ টার্ম থাকলেই অপশনগুলো দেখাবে
...
//       {isOpen && (
//         <div className={`absolute left-0 mt-2 w-64 rounded-lg shadow-xl border z-50 p-3 ${
//           isDark ? "bg-[#1F2937] border-gray-700 shadow-black/40" : "bg-white border-gray-200 shadow-gray-200"
//         }`}>
//           {/* Search Bar */}
//           <div className="relative mb-3">
//             <Search className="absolute left-2 top-2.5 text-gray-400" size={14} />
...
//               autoFocus
//               placeholder={`Search ${label}...`}
//               className={`w-full pl-8 pr-3 py-2 text-xs rounded-md border outline-none focus:border-orange-500 ${
...
//               ))
//             ) : (
//               <p className="text-center text-[10px] text-gray-400 py-2">No results found</p>
//             )}
//           </div>
...
//               className={`text-[10px] font-bold ${searchTerm.trim() === "" ? "text-gray-500 cursor-not-allowed" : "text-orange-500 hover:underline"}`}
//             >
//               Select All
//             </button>
//             <button
...
//                       <div className="flex items-center gap-3">
//                         <div className={`w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-bold shadow-sm border ${isDark ? "bg-orange-500/10 text-orange-400 border-orange-500/20" : "bg-orange-50 text-orange-600 border-orange-100"}`}>
//                           {project.assignee ? project.assignee.split(' ').map(n => n[0]).join('').toUpperCase() : "NA"}
//                         </div>
//                         <div className="flex flex-col text-left">
//                           <span className={`text-sm font-semibold ${isDark ? "text-gray-200" : "text-gray-700"}`}>
//                             {project.assignee}
...
const FilterDropdown = ({ columnKey, label, data, onFilterChange, isDark }) => {
  ...
  return (
    <div className="relative inline-block ml-1" ref={dropdownRef}>
      <Filter
        size={13}
        className={`cursor-pointer hover:text-orange-500 transition-colors ${isOpen ? "text-orange-500" : "text-gray-400"}`}
        onClick={() => setIsOpen(!isOpen)}
      />
...
export default function ProjectList({ searchQuery = "" }) {
  ...
  const filteredProjects = projects.filter((project) => {
    ...
    const globalMatch =
      ...
      (project.deadline && project.deadline.toLowerCase().includes(query)) ||
      (project.value && project.value.toLowerCase().includes(query)) ||
      (project.assignee && project.assignee.toLowerCase().includes(query)) ||
      teamNames.includes(query);

    // Column-wise Multi-select Filter Logic
    const columnMatch = Object.keys(activeFilters).every(key => {
      if (!activeFilters[key] || activeFilters[key].length === 0) return true;
      return activeFilters[key].includes(String(project[key]));
    });
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className={`text-xs font-bold uppercase tracking-wider border-b ${isDark ? "border-gray-800 text-gray-400" : "border-gray-100 text-gray-500"}`}>
                ...
                <th className="p-4 whitespace-nowrap">
                  Deadline <FilterDropdown columnKey="deadline" label="Deadline" data={projects} onFilterChange={handleFilterChange} isDark={isDark} />
                </th>
                <th className="p-4 whitespace-nowrap">
                  Value <FilterDropdown columnKey="value" label="Value" data={projects} onFilterChange={handleFilterChange} isDark={isDark} />
                </th>
                <th className="p-4 whitespace-nowrap">
                  Assigned Team <FilterDropdown columnKey="assignee" label="Team" data={projects} onFilterChange={handleFilterChange} isDark={isDark} />
                </th>
                <th className="p-4 text-center">Actions</th>
              </tr>
            </thead>
